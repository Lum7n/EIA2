"use strict";
var CanvasVirusus_V2;
(function (CanvasVirusus_V2) {
    ...
    function init(_event) {
        ...
        viewportHeight += -180;
        viewportWidth += -140;
        let heightAdjusted = viewportHeight + "px";
        let widthAdjusted = viewportWidth + "px";
        ...
        let midpoint = { x: viewportWidth / 2, y: viewportHeight / 2 };
        ...
        function drawCells(_midpoint, _canvasSize) {
            let nCells = 6;
            let cell = new Path2D();
            let nucleus = new Path2D();
            ...
            for (let drawn = 0; drawn < nCells; drawn++) {
                crc2.save();
                let x = (Math.random() - 0.5) * _canvasSize.x;
                let y = (Math.random() - 0.5) * _canvasSize.y;
                crc2.translate(x, y);
                crc2.fill(cell);
                crc2.stroke(cell);
                crc2.restore();
        ...
        function drawViruses(_midpoint, _canvasSize) {
            let nViruses = 8;
            let membraneProtein = new Path2D();
            let bodyVirus = new Path2D();
            ...
            crc2.lineWidth = 3;
            for (let drawn = 0; drawn < nViruses; drawn++) {
                crc2.save();
                let x = (Math.random() - 0.5) * _canvasSize.x;
                let y = (Math.random() - 0.5) * _canvasSize.y;
                crc2.translate(x, y);
        ...
        function drawPlasmaCells(_midpoint, _canvasSize) {
            let nCells = 2;
            let cell = new Path2D();
            let nucleus = new Path2D();
            let nAntibodys = 6;
            let antibodys = new Path2D();
            ...
            for (let drawn = 0; drawn < nCells; drawn++) {
                ...
                crc2.stroke(nucleus);
                crc2.restore();
                for (let drawn = 0; drawn < nAntibodys; drawn++) {
                    crc2.save();
                    let xA = (Math.random() - 0.5) * 50;
                    let yA = (Math.random() - 0.5) * 50;
                    x = xA + x;
                    y = yA + y;
        ...
        function drawMacrophages(_midpoint, _canvasSize) {
            let nCells = 3;
            let cell = new Path2D();
            let nucleus = new Path2D();
...
})(CanvasVirusus_V2 || (CanvasVirusus_V2 = {}));
